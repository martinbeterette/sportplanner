...
$sql = "SELECT  
                    ...
                    JOIN 
                        usuarios 
                    ON
                        usuarios.id_usuario = empleado.rela_usuario
                    WHERE (nombre                LIKE ?
                            OR apellido                 LIKE ?
...
while ($row = $result->fetch_assoc()) {
    ...
    $tabla .= '<td class="acciones">' . '<a href="formulario_creacion_empleado/includes/modificar.php?id_empleado=' . $row['id_empleado'] . '&id_sucursal='. $id_sucursal .'">
                    <img src="' . BASE_URL . 'assets/icons/icons8-editar.svg' . '"></a>' . 
                '</td>';

    $tabla .= '<td class="acciones">' . '<accion href="" class="eliminar" id_sucursal="' . $id_sucursal . '" valor="' . $row['id_empleado'] . '"><img src="' . BASE_URL . 'assets/icons/icons8-basura-llena.svg' . '"></accion>' . '</td>';
...
$sql_count = "SELECT COUNT(id_empleado) as total 
              FROM
                    ...
                    ON
                        documento.rela_persona = persona.id_persona
                    JOIN usuarios ON usuarios.id_usuario = empleado.rela_usuario
                    WHERE 
                    (
                        nombre                      LIKE ?
                        OR apellido                 LIKE ?

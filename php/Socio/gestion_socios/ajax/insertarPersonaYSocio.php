...
$cantidad_meses = intval($_REQUEST['cantidad_meses']);
$id_membresia = intval($_REQUEST['id_membresia']);
$id_complejo = intval($_REQUEST['id_complejo']); // Asegúrate de pasar este dato desde el frontend
...
try {
    ...
    $stmt_socio = $conexion->prepare(
        "INSERT INTO socio 
        (fecha_alta,fecha_afiliacion,fecha_expiracion, rela_complejo, rela_membresia,rela_persona)
        VALUES(
                CURDATE(),/*Fecha de alta*/
                CURDATE(),
                DATE_ADD(NOW(), INTERVAL ? MONTH),/*Fecha de expiracion*/ 
                ?,/* rela complejo*/
                ?,/* rela membresia*/
                ?/*rela persona*/
        )
